...
def count_rows():
    ...
    for i in range(10):
        ...
        # 조건 수정해야함
        dollido_wallet = DollidoLstId.objects.filter(Q(lstYmd=day) & Q(lstcontent__contains="지갑"))
        dollido_wallet_cnt = dollido_wallet.count()
        
        dollido_phone = DollidoLstId.objects.filter(Q(lstYmd=day) & Q(lstcontent__contains="스마트폰"))
